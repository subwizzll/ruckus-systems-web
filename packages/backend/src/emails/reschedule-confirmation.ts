export const RESCHEDULE_CONFIRMATION_EMAIL_SUBJECT = "Appointment rescheduled - Ruckus Systems";
 ...
 */
export function buildRescheduleConfirmationEmailHtml(input: RescheduleConfirmationEmailInput): string {
  const { clientFirstName, serviceTitle, newStartTimeIso, timezone, rescheduleUrl, cancelUrl } = input;
  const slotLabel = formatSlotDisplay(newStartTimeIso, timezone);
...
<head>
  ...
  <title>${escapeHtml(RESCHEDULE_CONFIRMATION_EMAIL_SUBJECT)}</title>
        ...
        <table role="presentation" width="100%" style="max-width:520px;background-color:#eee8d5;border:1px solid #93a1a1;border-radius:8px;overflow:hidden;" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td style="padding:24px 24px 8px 24px;background-color:#fdf6e3;">
              ...
              <p style="margin:0 0 12px 0;font-family:'Space Grotesk','Segoe UI',system-ui,sans-serif;font-size:16px;line-height:1.55;color:#586e75;">
                Your <strong style="color:#073642;">${escapeHtml(serviceTitle)}</strong> has a new time:
              </p>
              ...
              <p style="margin:0 0 16px 0;font-family:'Space Grotesk','Segoe UI',system-ui,sans-serif;font-size:16px;line-height:1.55;color:#586e75;">
                Use the links below if you need to adjust again or cancel.
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding:0 24px 24px 24px;background-color:#fdf6e3;">
              <table role="presentation" cellspacing="0" cellpadding="0" width="100%">
                <tr>
                  <td style="padding:0 0 10px 0;">
                    <a href="${escapeHtml(rescheduleUrl)}" style="display:inline-block;padding:12px 20px;background-color:#268bd2;color:#fdf6e3;font-family:'Space Grotesk','Segoe UI',system-ui,sans-serif;font-size:15px;font-weight:600;text-decoration:none;border-radius:6px;">
                      Reschedule
                    </a>
                  </td>
                </tr>
                <tr>
                  <td>
                    <a href="${escapeHtml(cancelUrl)}" style="display:inline-block;padding:12px 20px;background-color:transparent;color:#cb4b16;font-family:'Space Grotesk','Segoe UI',system-ui,sans-serif;font-size:15px;font-weight:600;text-decoration:underline;border-radius:6px;border:1px solid #cb4b16;">
                      Cancel booking
                    </a>
                  </td>
                </tr>
              ...
              <p style="margin:20px 0 0 0;font-family:'Space Grotesk','Segoe UI',system-ui,sans-serif;font-size:12px;line-height:1.5;color:#657b83;word-break:break-all;">
                Reschedule: <a href="${escapeHtml(rescheduleUrl)}" style="color:#268bd2;">${escapeHtml(rescheduleUrl)}</a><br />
                Cancel: <a href="${escapeHtml(cancelUrl)}" style="color:#268bd2;">${escapeHtml(cancelUrl)}</a>
              </p>
            </td>
          </tr>
          <tr>
